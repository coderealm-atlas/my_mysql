...
#define __BOOST_SML_REQUIRES(...) \
  typename aux::enable_if<__VA_ARGS__, int>::type = 0
namespace aux {
...
template <class T>
struct is_empty
    : aux::integral_constant<bool, sizeof(is_empty_base<T, none_type>) ==
                                       sizeof(none_type)> {};
template <class>
...
struct join<type_list<T1s...>, type_list<T2s...>, type_list<T3s...> >
    : type_list<T1s..., T2s..., T3s...> {};
...
struct is_unique<type<Rs...>, T, Ts...>
    : conditional_t<is_base_of<type<T>, inherit<type<Rs>...> >::value,
                    false_type, is_unique<type<Rs..., T>, Ts...> > {};
template <class... Ts>
...
struct pool_type_impl<
    T&, aux::enable_if_t<aux::is_constructible<T>::value &&
                         aux::is_constructible<T, const T&>::value> >
    : pool_type_base {
  constexpr explicit pool_type_impl(T& value) : value{value} {}
  template <class TObject>
  constexpr explicit pool_type_impl(TObject value)
      : value_{value}, value{value_} {}
  template <class TObject>
...
struct missing_ctor_parameter {
  ...
  template <class U,
            __BOOST_SML_REQUIRES(!aux::is_base_of<pool_type_base, U>::value &&
                                 aux::is_constructible<U>::value)>
  ...
  template <
      class TMissing,
      __BOOST_SML_REQUIRES(!aux::is_base_of<pool_type_base, TMissing>::value)>
  constexpr operator TMissing&() const {
    static_assert(missing_ctor_parameter<TMissing>::value,
                  "State Machine is missing a constructor parameter! Check out "
                  "the `missing_ctor_parameter` error to see the "
                  "missing type.");
  }
#endif
};
...
constexpr bool would_instantiate_missing_ctor_parameter() {
  return is_same<missing_ctor_parameter<T>,
                 decltype(try_get<T>(aux::declval<TPool>()))>::value;
}
template <class T, class TPool>
...
struct pool : pool_type<Ts>... {
  ...
  constexpr pool(init, const pool<TArgs...>& p)
      : pool_type<Ts>(
            try_get<aux::remove_const_t<
                aux::remove_reference_t<aux::remove_pointer_t<Ts> > > >(
                &p))... {}
  template <class... TArgs>
...
struct zero_wrapper<R (TBase::*)(TArgs...), T> {
  ...
  constexpr auto operator()(TBase& self, TArgs... args) {
...
struct zero_wrapper_impl<TExpr, type_list<TArgs...> > {
  constexpr auto operator()(TArgs... args) const {
    return reinterpret_cast<const TExpr&>(*this)(args...);
  }
  __BOOST_SML_ZERO_SIZE_ARRAY(byte);
};
...
#if defined(_MSC_VER) && !defined(__clang__)
  return detail::get_type_name<T, 65>(
      __FUNCSIG__, make_index_sequence<sizeof(__FUNCSIG__) - 65 - 8>{});
#elif defined(__clang__) && (__clang_major__ >= 12)
  return detail::get_type_name<T, 50>(
      __PRETTY_FUNCTION__,
      make_index_sequence<sizeof(__PRETTY_FUNCTION__) - 50 - 2>{});
#elif defined(__clang__)
  return detail::get_type_name<T, 63>(
      __PRETTY_FUNCTION__,
      make_index_sequence<sizeof(__PRETTY_FUNCTION__) - 63 - 2>{});
#elif defined(__GNUC__)
  return detail::get_type_name<T, 69>(
      __PRETTY_FUNCTION__,
      make_index_sequence<sizeof(__PRETTY_FUNCTION__) - 69 - 2>{});
#elif defined(__ICCARM__)
  return detail::get_type_name<T, 72>(
      __PRETTY_FUNCTION__,
      make_index_sequence<sizeof(__PRETTY_FUNCTION__) - 72 - 2>{});
#endif
}
...
struct defer_queue
    : aux::pair<back::policies::defer_queue_policy__, defer_queue<T> > {
  template <class U>
  using rebind = T<U>;
  using flag = bool;
};
...
class queue_event {
  ...
  template <class T>
  constexpr static void move_impl(aux::byte (&data)[size],
                                  queue_event&& other) {
 ...
 public:
  ...
  constexpr queue_event(queue_event&& other)
      : id(other.id), dtor(other.dtor), move(other.move) {
...
struct queue_handler : queue_event_call<TEvents>... {
  ...
  constexpr explicit queue_handler(TQueue& queue)
      : queue_event_call<TEvents>(queue_handler::push_impl<TQueue, TEvents>)...,
        queue_{&queue} {}
  template <class TEvent>
...
struct deque_handler : queue_event_call<TEvents>... {
  ...
  constexpr explicit deque_handler(TDeque& deque)
      : queue_event_call<TEvents>(deque_handler::push_impl<TDeque, TEvents>)...,
        deque_{&deque} {}
  template <class TEvent>
  constexpr void operator()(const TEvent& event) {
...
struct exception : internal_event {
  ...
  constexpr explicit exception(const TException& exception = {})
      : exception_(exception) {}
  const TException& exception_;
};
template <class T, class TEvent = T>
struct unexpected_event : internal_event, unexpected {
  constexpr explicit unexpected_event(const TEvent& event = {})
      : event_(event) {}
  const TEvent& event_;
};
template <class TEvent>
...
template <class TEvent>
using get_event =
    aux::conditional_t<aux::is_base_of<internal_event, TEvent>::value,
                       aux::type_list<>, aux::type_list<TEvent> >;
...
struct get_sub_internal_events_impl {
  using type =
      aux::conditional_t<aux::is_base_of<internal_event, TEvent>::value,
                         aux::type_list<TEvent>, aux::type_list<> >;
};
template <class T, class TEvent>
struct get_sub_internal_events_impl<sm<T>, TEvent> {
  using type = aux::join_t<aux::type_list<TEvent>,
                           typename sm_impl<T>::sub_internal_events_t>;
};
template <class... Ts>
using get_all_events = aux::join_t<
    typename get_all_events_impl<typename Ts::src_state, typename Ts::dst_state,
                                 typename Ts::event>::type...>;
template <class... Ts>
using get_sub_internal_events =
    aux::join_t<typename get_sub_internal_events_impl<
                    typename Ts::src_state, typename Ts::event>::type...,
                typename get_sub_internal_events_impl<
                    typename Ts::dst_state, typename Ts::event>::type...>;
template <class... Ts>
...
template <class... Ts>
using get_states = aux::join_t<
    aux::type_list<typename Ts::src_state, typename Ts::dst_state>...>;
template <class... Ts>
using get_initial_states = aux::join_t<typename aux::conditional<
    Ts::initial, aux::type_list<typename Ts::src_state>,
    aux::type_list<> >::type...>;
template <class... Ts>
using get_history_states = aux::join_t<typename aux::conditional<
    !Ts::history && Ts::initial, aux::type_list<typename Ts::src_state>,
    aux::type_list<> >::type...>;
template <class>
struct get_sub_sm : aux::type_list<> {};
template <class T>
struct get_sub_sm<sm<T> >
    : aux::join_t<aux::type_list<T>, typename sm<T>::state_machines> {};
template <class... Ts>
using get_sub_sms = aux::join_t<typename get_sub_sm<Ts>::type...>;
template <class... Ts>
using get_sm_t = aux::type_list<typename Ts::sm...>;
template <class... Ts>
using get_non_empty_t = aux::join_t<typename aux::conditional<
    aux::is_empty<Ts>::value, aux::type_list<>, aux::type_list<Ts> >::type...>;
template <class... Ts>
...
struct sub_sm<sm_impl<sm_policy<T> > > {
  ...
  constexpr static sm_impl<sm_policy<T, aux::identity<U>, TPolicies...> >& get(
      aux::pool_type<sm_impl<sm_policy<T, aux::identity<U>, TPolicies...> > >*
          object) {
    return static_cast<aux::pool_type<
        sm_impl<sm_policy<T, aux::identity<U>, TPolicies...> > >&>(*object)
        .value;
  }
  template <class... TPolicies>
  constexpr static sm_impl<sm_policy<T, TPolicies...> >& get(
      aux::pool_type<sm_impl<sm_policy<T, TPolicies...> > >* object) {
    return static_cast<aux::pool_type<sm_impl<sm_policy<T, TPolicies...> > >&>(
               *object)
        .value;
  }
  template <class... TPolicies>
  constexpr static const sm_impl<sm_policy<T, TPolicies...> >& cget(
      const aux::pool_type<sm_impl<sm_policy<T, TPolicies...> > >* object) {
    return static_cast<
...
struct sub_sm<sm_impl<sm_policy<T, aux::identity<U> > > > {
  ...
  template <class... TPolicies>
  constexpr static const sm_impl<sm_policy<T, aux::identity<U>, TPolicies...> >&
  cget(const aux::pool_type<
       sm_impl<sm_policy<T, aux::identity<U>, TPolicies...> > >* object) {
    return static_cast<const aux::pool_type<
        sm_impl<sm_policy<T, aux::identity<U>, TPolicies...> > >&>(*object)
        .value;
  }
};
...
struct convert_to_sm<T, aux::type_list<Ts...> > {
  using type =
      aux::type_list<sm_impl<T>, sm_impl<typename T::template rebind<Ts> >...>;
};
}  // namespace back
namespace back {
template <class>
...
struct transitions<T, Ts...> {
  ...
  constexpr static bool execute(const TEvent& event, SM& sm, TDeps& deps,
                                TSubs& subs,
                                typename SM::state_t& current_state) {
    if (aux::get<T>(sm.transitions_)
...
struct transitions<T> {
  ...
  constexpr static bool execute(const TEvent& event, SM& sm, TDeps& deps,
                                TSubs& subs,
                                typename SM::state_t& current_state) {
    return aux::get<T>(sm.transitions_)
        .execute(event, sm, deps, subs, current_state,
                 typename SM::has_entry_exits{});
  }
};
...
struct transitions<aux::true_type> {
  template <class TEvent, class SM, class TDeps, class TSubs>
  static bool execute(const TEvent& event, SM& sm, TDeps& deps, TSubs& subs,
                      typename SM::state_t& current_state) {
    sm.process_internal_event(unexpected_event<TEvent>{event}, deps, subs,
                              current_state);
    return false;
  }
};
...
struct transitions<aux::false_type> {
  template <class TEvent, class SM, class TDeps, class TSubs>
  constexpr static bool execute(const TEvent&, SM&, TDeps&, TSubs&,
                                typename SM::state_t&) {
    return false;
  }
};
...
struct transitions_sub<sm<Tsm>, T, Ts...> {
  ...
  constexpr static bool execute(const anonymous& event, SM& sm, TDeps& deps,
                                TSubs& subs,
                                typename SM::state_t& current_state) {
    if (sub_sm<sm_impl<Tsm> >::cget(&subs).is_terminated()) {
      return transitions<T, Ts...>::execute(event, sm, deps, subs,
                                            current_state);
    } else {
  ...
  template <class TEvent, class SM, class TDeps, class TSubs>
  constexpr static bool execute_impl(const TEvent& event, SM& sm, TDeps& deps,
                                     TSubs& subs,
                                     typename SM::state_t& current_state) {
    const auto handled =
        sub_sm<sm_impl<Tsm> >::get(&subs).process_event(event, deps, subs);
    return handled ? handled
                   : transitions<T, Ts...>::execute(event, sm, deps, subs,
                                                    current_state);
  }
  template <class _, class TEvent, class SM, class TDeps, class TSubs>
  constexpr static bool execute_impl(const back::on_entry<_, TEvent>& event,
                                     SM& sm, TDeps& deps, TSubs& subs,
                                     typename SM::state_t& current_state) {
  ...
  template <class _, class TEvent, class SM, class TDeps, class TSubs>
  constexpr static bool execute_impl(const back::on_exit<_, TEvent>& event,
                                     SM& sm, TDeps& deps, TSubs& subs,
                                     typename SM::state_t& current_state) {
    sub_sm<sm_impl<Tsm> >::get(&subs).process_event(event, deps, subs);
    transitions<T, Ts...>::execute(event, sm, deps, subs, current_state);
...
struct transitions_sub<sm<Tsm> > {
  template <class TEvent, class SM, class TDeps, class TSubs>
  constexpr static bool execute(const TEvent& event, SM&, TDeps& deps,
                                TSubs& subs, typename SM::state_t&) {
    return sub_sm<sm_impl<Tsm> >::get(&subs).process_event(event, deps, subs);
  }
  template <class, class SM, class TDeps, class TSubs>
  constexpr static bool execute(const anonymous&, SM&, TDeps&, TSubs&,
                                typename SM::state_t&) {
    return false;
  }
};
...
struct get_mapping<event<E>, event<E>, T, R>
    : aux::type_list<event_mappings<
          E,
          aux::apply_t<unique_mappings_t,
                       aux::join_t<typename R::types, typename T::types> > > > {
};
template <class S, class T, class R>
struct get_mapping<state<S>, state<S>, T, R>
    : aux::type_list<state_mappings<
          S, aux::join_t<typename R::types, typename T::types> > > {};
template <class T, class... Ts>
struct extend_mapping
    : aux::join_t<
          typename get_mapping<typename T::element_type,
                               typename Ts::element_type, T, Ts>::type...> {};
template <class T, class... Ts>
using extend_mapping_t =
    aux::apply_t<aux::inherit, typename extend_mapping<T, Ts...>::type>;
template <bool, class, class...>
struct conditional_mapping;
template <class T1, class T2, class... Rs, class... Ts>
struct conditional_mapping<true, aux::type<T1, aux::inherit<Rs...> >, T2,
                           Ts...> {
  using type = unique_mappings_impl<
      aux::type<aux::inherit<T1>, extend_mapping_t<T2, Rs...> >, Ts...>;
};
template <class T1, class T2, class... Rs, class... Ts>
struct conditional_mapping<false, aux::type<T1, aux::inherit<Rs...> >, T2,
                           Ts...> {
  using type = unique_mappings_impl<
      aux::type<aux::inherit<T1, aux::type<typename T2::element_type> >,
                aux::inherit<T2, Rs...> >,
      Ts...>;
};
template <class T1, class T2, class... Rs, class... Ts>
struct unique_mappings_impl<aux::type<T1, aux::inherit<Rs...> >, T2, Ts...>
    : conditional_mapping<
          aux::is_base_of<aux::type<typename T2::element_type>, T1>::value,
          aux::type<T1, aux::inherit<Rs...> >, T2, Ts...>::type {};
template <class T1, class Rs>
struct unique_mappings_impl<aux::type<T1, Rs> >
    : aux::apply_t<aux::inherit, Rs> {};
template <class... Ts>
struct unique_mappings
    : unique_mappings_impl<aux::type<aux::none_type, aux::inherit<> >, Ts...> {
};
template <class T>
...
struct mappings<aux::pool<Ts...> >
    : unique_mappings_t<event_mappings<
          typename Ts::event,
          aux::inherit<state_mappings<typename Ts::src_state,
                                      aux::type_list<Ts> > > >...> {};
template <class T>
...
transitions<Ts...> get_state_mapping_impl(
    state_mappings<T, aux::type_list<Ts...> >*);
template <class T, class TMappings, class TUnexpected>
struct get_state_mapping {
  using type = aux::conditional_t<
      aux::is_same<decltype(get_state_mapping_impl<T, TUnexpected>(
                       (TMappings*)0)),
                   transitions<TUnexpected> >::value,
...
template <class T, class... Ts>
transitions_sub<T, Ts...> get_sub_state_mapping_impl(
    state_mappings<T, aux::type_list<Ts...> >*);
template <class T, class TMappings, class TUnexpected>
struct get_state_mapping<sm<T>, TMappings, TUnexpected> {
  using sub_sm_mapping =
      decltype(get_sub_state_mapping_impl<sm<T> >((TMappings*)0));
  using fallback_event_mapping =
      decltype(get_state_mapping_impl<_, TUnexpected>((TMappings*)0));
  struct type : sub_sm_mapping {
    template <class TEvent, class TSm, class TDeps, class TSubs>
    constexpr static bool execute(const TEvent& event, TSm& sm, TDeps& deps,
                                  TSubs& subs,
                                  typename TSm::state_t& current_state) {
      return sub_sm_mapping::template execute<TEvent, TSm, TDeps, TSubs>(
                 event, sm, deps, subs, current_state) ||
             fallback_event_mapping::template execute<TEvent, TSm, TDeps,
                                                      TSubs>(
                 event, sm, deps, subs, current_state);
    }
  };
...
unique_mappings_t<T1Mappings..., T2Mappings...> get_event_mapping_impl(
    event_mappings<T, aux::inherit<T1Mappings...> >*,
    event_mappings<_, aux::inherit<T2Mappings...> >*);
template <class T, class TMappings>
struct get_event_mapping_impl_helper
    : aux::conditional<aux::is_same<transitions<aux::true_type>,
                                    decltype(get_event_mapping_impl<_>(
                                        (TMappings*)0))>::value,
                       decltype(get_event_mapping_impl<T>((TMappings*)0)),
                       decltype(get_event_mapping_impl<T>(
                           (TMappings*)0, (TMappings*)0))>::type {};
template <class T, class TMappings>
...
struct get_event_mapping_impl_helper<unexpected_event<T1, T2>, TMappings>
    : decltype(get_event_mapping_impl<unexpected_event<T1, T2> >(
          (TMappings*)0)) {};
template <class T1, class T2, class TMappings>
...
struct jump_table {
  template <int, class TMappings, class sm_impl, class State, class TEvent,
            class TDeps, class TSubs>
  constexpr static bool dispatch(sm_impl&, State&, const TEvent&, TDeps&,
                                 TSubs&, const aux::type_list<>&) {
    return false;
  }
  template <int, class TMappings, class sm_impl, class State, class TEvent,
            class TDeps, class TSubs, class... TStates>
  static bool dispatch(sm_impl& self, State& current_state, const TEvent& event,
                       TDeps& deps, TSubs& subs,
                       const aux::type_list<TStates...>&) {
    using dispatch_table_t =
        bool (*)(const TEvent&, sm_impl&, TDeps&, TSubs&, State&);
    constexpr static dispatch_table_t
        dispatch_table[__BOOST_SML_ZERO_SIZE_ARRAY_CREATE(sizeof...(TStates))] =
            {&get_state_mapping_t<TStates, TMappings,
                                  typename sm_impl::has_unexpected_events>::
                 template execute<TEvent, sm_impl, TDeps, TSubs>...};
    return dispatch_table[current_state](event, self, deps, subs,
                                         current_state);
  }
};
struct branch_stm {
  ...
  template <int N, class TMappings, class sm_impl, class State, class TEvent,
            class TDeps, class TSubs, class TState, class... TStates>
  constexpr static bool dispatch(sm_impl& self, State& current_state,
                                 const TEvent& event, TDeps& deps, TSubs& subs,
                                 const aux::type_list<TState, TStates...>&) {
    return current_state == N
               ? get_state_mapping_t<TState, TMappings,
                                     typename sm_impl::has_unexpected_events>::
                     template execute<TEvent, sm_impl, TDeps, TSubs>(
                         event, self, deps, subs, current_state)
               : dispatch<N + 1, TMappings>(self, current_state, event, deps,
                                            subs, aux::type_list<TStates...>{});
  }
};
struct switch_stm {
  template <int, class TMappings, class sm_impl, class State, class TEvent,
  ...
  constexpr static bool dispatch(sm_impl& self, State& current_state,
    ...
    switch (current_state) {
      case N:
        return get_state_mapping_t<TState, TMappings,
                                   typename sm_impl::has_unexpected_events>::
            template execute<TEvent, sm_impl, TDeps, TSubs>(
                event, self, deps, subs, current_state);
      default:
        return dispatch<N + 1, TMappings>(self, current_state, event, deps,
                                          subs, aux::type_list<TStates...>{});
    }
  }
};
...
struct fold_expr {
  template <class TMappings, int... Ns, class sm_impl, class State,
            class TEvent, class TDeps, class TSubs, class... TStates>
  constexpr static bool dispatch_impl(sm_impl& self, State& current_state,
  ...
  constexpr static bool dispatch(sm_impl& self, State& current_state,
    ...
    return dispatch_impl<TMappings>(
        self, current_state, aux::make_index_sequence<sizeof...(TStates)>{},
        event, deps, subs, states);
  }
};
#endif
}  // namespace policies
...
constexpr void log_process_event(const aux::type<no_policy>&, TDeps&,
                                 const TEvent&) {}
template <class SM, class TLogger, class TDeps, class TEvent>
constexpr void log_process_event(const aux::type<TLogger>&, TDeps& deps,
                                 const TEvent& event) {
  return static_cast<aux::pool_type<TLogger&>&>(deps)
      .value.template log_process_event<SM>(event);
}
template <class, class TDeps, class TSrcState, class TDstState>
constexpr void log_state_change(const aux::type<no_policy>&, TDeps&,
                                const TSrcState&, const TDstState&) {}
template <class SM, class TLogger, class TDeps, class TSrcState,
          class TDstState>
constexpr void log_state_change(const aux::type<TLogger>&, TDeps& deps,
                                const TSrcState&, const TDstState&) {
  return static_cast<aux::pool_type<TLogger&>&>(deps)
      .value.template log_state_change<SM>(get_state_name_t<TSrcState>{},
                                           get_state_name_t<TDstState>{});
}
template <class, class TDeps, class TAction, class TEvent>
constexpr void log_action(const aux::type<no_policy>&, TDeps&, const TAction&,
                          const TEvent&) {}
template <class SM, class TLogger, class TDeps, class TAction, class TEvent>
constexpr void log_action(const aux::type<TLogger>&, TDeps& deps,
                          const TAction& action, const TEvent& event) {
  return static_cast<aux::pool_type<TLogger&>&>(deps)
      .value.template log_action<SM>(action, event);
}
template <class SM, class TLogger, class TDeps, class TAction, class TEvent>
constexpr void log_action(const aux::type<TLogger>&, TDeps& deps,
                          ...
                          const TEvent& event) {
  return static_cast<aux::pool_type<TLogger&>&>(deps)
      .value.template log_action<SM>(action.get(), event);
}
template <class, class TDeps, class TGuard, class TEvent>
constexpr void log_guard(const aux::type<no_policy>&, TDeps&, const TGuard&,
                         const TEvent&, bool) {}
template <class SM, class TLogger, class TDeps, class TGuard, class TEvent>
constexpr void log_guard(const aux::type<TLogger>&, TDeps& deps,
                         const TGuard& guard, const TEvent& event,
                         bool result) {
  return static_cast<aux::pool_type<TLogger&>&>(deps)
      .value.template log_guard<SM>(guard, event, result);
}
template <class SM, class TLogger, class TDeps, class TGuard, class TEvent>
constexpr void log_guard(const aux::type<TLogger>&, TDeps& deps,
                         const aux::zero_wrapper<TGuard>& guard,
                         const TEvent& event, bool result) {
  return static_cast<aux::pool_type<TLogger&>&>(deps)
      .value.template log_guard<SM>(guard.get(), event, result);
}
}  // namespace policies
}  // namespace back
namespace back {
...
struct process_queue
    : aux::pair<back::policies::process_queue_policy__, process_queue<T> > {
  template <class U>
  using rebind = T<U>;
};
}  // namespace policies
...
struct dont_instantiate_statemachine_class
    : aux::pair<dont_instantiate_statemachine_class_policy__,
...
struct should_not_instantiate_statemachine_class
    : integral_constant<
          bool,
          is_same<typename Tsm::dont_instantiate_statemachine_class_policy,
...
struct should_not_subclass_statemachine_class
    : integral_constant<
          bool, is_empty<typename Tsm::sm>::value ||
...
struct sm_policy {
  static_assert(aux::is_same<aux::remove_reference_t<SM>, SM>::value,
                "SM type can't have qualifiers");
  using sm = SM;
  using thread_safety_policy =
      decltype(get_policy<no_policy, policies::thread_safety_policy__>(
          (aux::inherit<TPolicies...>*)0));
  using defer_queue_policy =
      decltype(get_policy<no_policy, policies::defer_queue_policy__>(
          (aux::inherit<TPolicies...>*)0));
  using process_queue_policy =
      decltype(get_policy<no_policy, policies::process_queue_policy__>(
          (aux::inherit<TPolicies...>*)0));
  using logger_policy =
      decltype(get_policy<no_policy, policies::logger_policy__>(
          (aux::inherit<TPolicies...>*)0));
  using testing_policy =
      decltype(get_policy<no_policy, policies::testing_policy__>(
          (aux::inherit<TPolicies...>*)0));
  using dont_instantiate_statemachine_class_policy =
      decltype(get_policy<
               no_policy,
               policies::dont_instantiate_statemachine_class_policy__>(
          (aux::inherit<TPolicies...>*)0));
  using default_dispatch_policy = policies::switch_stm;
  using dispatch_policy =
      decltype(get_policy<default_dispatch_policy, policies::dispatch_policy__>(
          (aux::inherit<TPolicies...>*)0));
  template <class T>
...
template <class T>
aux::false_type test_callable(
    aux::non_type<void (callable_fallback::*)(), &T::operator()>*);
template <class>
aux::true_type test_callable(...);
template <class, class T>
struct callable
    : decltype(test_callable<aux::inherit<
                   aux::conditional_t<__is_class(T), T, aux::none_type>,
                   callable_fallback> >(0)) {};
...
namespace back {
template <class Tsm>
struct sm_impl : aux::conditional_t<
                     aux::should_not_subclass_statemachine_class<Tsm>::value,
                     aux::none_type, typename Tsm::sm> {
  ...
  using states_t =
      aux::apply_t<aux::unique_t, aux::apply_t<get_states, transitions_t> >;
  using states_ids_t = aux::apply_t<aux::type_id, states_t>;
  using initial_states_t =
      aux::apply_t<aux::unique_t,
                   aux::apply_t<get_initial_states, transitions_t> >;
  ...
  using has_history_states =
      aux::integral_constant<bool, aux::size<initial_states_t>::value !=
                                       aux::size<history_states_t>::value>;
  using sub_internal_events_t =
      aux::apply_t<aux::unique_t,
                   aux::apply_t<get_sub_internal_events, transitions_t> >;
  using events_t =
      aux::apply_t<aux::unique_t,
                   aux::join_t<sub_internal_events_t,
                               aux::apply_t<get_all_events, transitions_t> > >;
  using events_ids_t = aux::apply_t<aux::inherit, events_t>;
  using has_unexpected_events =
      typename aux::is_base_of<unexpected,
                               aux::apply_t<aux::inherit, events_t> >::type;
  using has_entry_exits =
      typename aux::is_base_of<entry_exit,
                               aux::apply_t<aux::inherit, events_t> >::type;
  using should_not_instantiate_statemachine_class_t =
      aux::should_not_instantiate_statemachine_class<Tsm>;
  ...
  using state_t = aux::conditional_t<(aux::size<states_t>::value > 0xFF),
  ...
  using exceptions =
      aux::apply_t<aux::unique_t, aux::apply_t<get_exceptions, events_t> >;
  using has_exceptions =
      aux::integral_constant<bool, (aux::size<exceptions>::value > 0)>;
#endif
  struct mappings : mappings_t<transitions_t> {};
  template <class TPool>
  constexpr sm_impl(aux::init, const TPool& p)
      : sm_impl{p, aux::should_not_subclass_statemachine_class<Tsm>{}} {}
  template <class TPool>
  constexpr sm_impl(const TPool& p, aux::false_type)
      : sm_t{aux::try_get<sm_t>(&p)}, transitions_{(*this)()} {
  ...
  constexpr decltype(auto) try_get_without_instantiating(const TPool& p) const {
    static_assert(
        !(should_not_instantiate_statemachine_class_t::value &&
          aux::would_instantiate_missing_ctor_parameter<sm_t, decltype(&p)>()),
        "When policy sml::dont_instantiate_statemachine_class is used, you "
        "have to provide a reference to an "
        "instance of the transition table type (boost::sml::sm< "
        "your_transition_table_type >) "
        "as well as a reference to instances of all sub-statemachine types as "
        "constructor parameters like this: \n"
        "boost::sml::sm< your_transition_table_type , "
        "sml::dont_instantiate_statemachine_class >{\n"
  ...
  template <class TPool>
  constexpr sm_impl(const TPool& p, aux::true_type)
      : transitions_{try_get_without_instantiating<sm_t>(p)} {
    initialize(typename sm_impl<Tsm>::initial_states_t{});
  }
  ...
  constexpr bool process_event(const TEvent& event, TDeps& deps, TSubs& subs) {
    ...
    do {
      ...
      } while (process_defer_events(deps, subs, changed,
                                    aux::type<defer_queue_t<TEvent> >{},
                                    events_t{}));
    } while (process_queued_events(deps, subs, queued_handled,
                                   aux::type<process_queue_t<TEvent> >{},
                                   events_t{}));
  ...
  constexpr void initialize(const aux::type_list<TStates...>&) {
    auto region = 0;
#if defined(__cpp_fold_expressions)
    ((current_state_[region++] =
          aux::get_id<state_t, TStates>((states_ids_t*)0)),
     ...);
#else
    (void)aux::swallow{0, (current_state_[region++] =
                               aux::get_id<state_t, TStates>((states_ids_t*)0),
                           0)...};
#endif
  }
  ...
  template <class TEvent, class TDeps, class TSubs, class... Ts,
            __BOOST_SML_REQUIRES(
                !aux::is_base_of<get_generic_t<TEvent>, events_ids_t>::value &&
                !aux::is_base_of<get_mapped_t<TEvent>, events_ids_t>::value &&
                !aux::is_same<get_event_t<TEvent>, initial>::value)>
  constexpr bool process_internal_events(const TEvent&, TDeps&, TSubs&,
                                         Ts&&...) {
    return false;
  }
  template <class TEvent, class TDeps, class TSubs, class... Ts,
            __BOOST_SML_REQUIRES(
                ...
                !aux::is_base_of<get_mapped_t<TEvent>, events_ids_t>::value &&
                aux::is_same<get_event_t<TEvent>, initial>::value)>
  constexpr bool process_internal_events(const TEvent& event, TDeps& deps,
                                         TSubs& subs) {
    policies::log_process_event<sm_t>(aux::type<logger_t>{}, deps, event);
#if BOOST_SML_DISABLE_EXCEPTIONS
    return process_event_impl<
        get_event_mapping_t<get_generic_t<TEvent>, mappings> >(
        event, deps, subs, states_t{}, aux::make_index_sequence<regions>{});
#else
    return process_event_except_impl<
        get_event_mapping_t<get_generic_t<TEvent>, mappings> >(
...
#if BOOST_SML_DISABLE_EXCEPTIONS
    return process_event_impl<
        get_event_mapping_t<get_mapped_t<TEvent>, mappings> >(
        event, deps, subs, states_t{}, aux::make_index_sequence<regions>{});
#else
    return process_event_except_impl<
        get_event_mapping_t<get_mapped_t<TEvent>, mappings> >(event, deps, subs,
                                                              has_exceptions{});
#endif
  }
  template <class TEvent, class TDeps, class TSubs, class... Ts,
            __BOOST_SML_REQUIRES(
                !aux::is_base_of<get_generic_t<TEvent>, events_ids_t>::value &&
                !aux::is_base_of<get_mapped_t<TEvent>, events_ids_t>::value)>
  constexpr bool process_internal_event(const TEvent&, TDeps&, TSubs&,
                                        Ts&&...) {
    return false;
  }
  template <class TEvent, class TDeps, class TSubs,
            __BOOST_SML_REQUIRES(
                !aux::is_base_of<get_generic_t<TEvent>, events_ids_t>::value)>
  constexpr bool process_internal_generic_event(const TEvent&, TDeps&, TSubs&,
                                                state_t&) {
    return false;
  }
  template <class TEvent, class TDeps, class TSubs,
            __BOOST_SML_REQUIRES(
                aux::is_base_of<get_generic_t<TEvent>, events_ids_t>::value)>
  constexpr bool process_internal_generic_event(const TEvent& event,
                                                TDeps& deps, TSubs& subs,
    ...
    return process_event_impl<
        get_event_mapping_t<get_generic_t<TEvent>, mappings> >(
        event, deps, subs, states_t{}, current_state);
#else
    return process_event_except_impl<
        get_event_mapping_t<get_generic_t<TEvent>, mappings> >(
  ...
  template <class TEvent, class TDeps, class TSubs,
            __BOOST_SML_REQUIRES(
                aux::is_base_of<get_generic_t<TEvent>, events_ids_t>::value &&
                !aux::is_base_of<get_mapped_t<TEvent>, events_ids_t>::value)>
  constexpr bool process_internal_event(const TEvent& event, TDeps& deps,
                                        TSubs& subs, state_t& current_state) {
  ...
  template <class TMappings, class TEvent, class TDeps, class TSubs,
            class... TStates>
  constexpr bool process_event_impl(const TEvent& event, TDeps& deps,
                                    TSubs& subs,
                                    const aux::type_list<TStates...>& states,
                                    aux::index_sequence<0>) {
    return dispatch_t::template dispatch<0, TMappings>(
        *this, current_state_[0], event, deps, subs, states);
  }
  template <class TMappings, class TEvent, class TDeps, class TSubs,
            class... TStates, int... Ns>
  constexpr bool process_event_impl(const TEvent& event, TDeps& deps,
                                    TSubs& subs,
                                    const aux::type_list<TStates...>& states,
                                    aux::index_sequence<Ns...>) {
    auto handled = false;
#if defined(__cpp_fold_expressions)
    ((handled |= dispatch_t::template dispatch<0, TMappings>(
          *this, current_state_[Ns], event, deps, subs, states)),
     ...);
#else
    (void)aux::swallow{
        0, (handled |= dispatch_t::template dispatch<0, TMappings>(
                *this, current_state_[Ns], event, deps, subs, states),
            0)...};
#endif
    return handled;
...
#if !BOOST_SML_DISABLE_EXCEPTIONS
  template <class TMappings, class TEvent, class TDeps, class TSubs>
  constexpr bool process_event_except_impl(const TEvent& event, TDeps& deps,
                                           TSubs& subs, aux::false_type) {
    return process_event_impl<TMappings>(event, deps, subs, states_t{},
  ...
  template <class TMappings, class TEvent, class TDeps, class TSubs>
  bool process_event_except_impl(const TEvent& event, TDeps& deps, TSubs& subs,
                                 state_t& current_state,
  ...
  constexpr bool process_defer_events(TDeps&, TSubs&, const bool,
                                      const aux::type<no_policy>&,
                                      const aux::type_list<TEvents...>&) {
    return false;
  }
  template <class TDeps, class TSubs, class TEvent>
  constexpr bool process_event_no_defer(TDeps& deps, TSubs& subs,
                                        const void* data) {
  ...
  bool process_defer_events(TDeps& deps, TSubs& subs, const bool handled,
                            const aux::type<TDeferQueue>&,
                            const aux::type_list<TEvents...>&) {
    bool processed_events = false;
    if (handled) {
      using dispatch_table_t = bool (sm_impl::*)(TDeps&, TSubs&, const void*);
      const static dispatch_table_t
          dispatch_table[__BOOST_SML_ZERO_SIZE_ARRAY_CREATE(
              sizeof...(TEvents))] = {
  ...
  template <class TDeps, class TSubs, class... TEvents>
  constexpr bool process_queued_events(TDeps&, TSubs&, bool&,
                                       const aux::type<no_policy>&,
                                       const aux::type_list<TEvents...>&) {
    return false;
  }
  template <class TDeps, class TSubs, class TEvent>
  constexpr bool process_event_no_queue(TDeps& deps, TSubs& subs,
                                        const void* data) {
    const auto& event = *static_cast<const TEvent*>(data);
    policies::log_process_event<sm_t>(aux::type<logger_t>{}, deps, event);
#if BOOST_SML_DISABLE_EXCEPTIONS
    return process_event_impl<get_event_mapping_t<TEvent, mappings> >(
        event, deps, subs, states_t{}, aux::make_index_sequence<regions>{});
#else
    return process_event_except_impl<get_event_mapping_t<TEvent, mappings> >(
        event, deps, subs, has_exceptions{});
#endif
  }
  template <class TDeps, class TSubs, class TDeferQueue, class... TEvents>
  bool process_queued_events(TDeps& deps, TSubs& subs, bool& queued_handled,
                             const aux::type<TDeferQueue>&,
                             const aux::type_list<TEvents...>&) {
    using dispatch_table_t = bool (sm_impl::*)(TDeps&, TSubs&, const void*);
    const static dispatch_table_t
        dispatch_table[__BOOST_SML_ZERO_SIZE_ARRAY_CREATE(sizeof...(TEvents))] =
            {&sm_impl::process_event_no_queue<TDeps, TSubs, TEvents>...};
    bool wasnt_empty = !process_.empty();
    while (!process_.empty()) {
      queued_handled &= (this->*dispatch_table[process_.front().id])(
          deps, subs, process_.front().data);
      process_.pop();
    }
    return wasnt_empty;
  }
  template <class TVisitor, class... TStates>
  constexpr void visit_current_states(const TVisitor& visitor,
                                      const aux::type_list<TStates...>&,
                                      aux::index_sequence<0>) const {
    using dispatch_table_t = void (*)(const TVisitor&);
    const dispatch_table_t dispatch_table[__BOOST_SML_ZERO_SIZE_ARRAY_CREATE(
        sizeof...(TStates))] = {&sm_impl::visit_state<TVisitor, TStates>...};
    dispatch_table[current_state_[0]](visitor);
  }
  template <class TVisitor, class... TStates, int... Ns>
  constexpr void visit_current_states(const TVisitor& visitor,
                                      const aux::type_list<TStates...>&,
                                      aux::index_sequence<Ns...>) const {
    using dispatch_table_t = void (*)(const TVisitor&);
    const dispatch_table_t dispatch_table[__BOOST_SML_ZERO_SIZE_ARRAY_CREATE(
        sizeof...(TStates))] = {&sm_impl::visit_state<TVisitor, TStates>...};
#if defined(__cpp_fold_expressions)
  ...
  constexpr bool is_terminated_impl(aux::index_sequence<0>) const {
    return current_state_[0] ==
           aux::get_id<state_t, terminate_state>((states_ids_t*)0);
  }
  template <int... Ns>
  constexpr bool is_terminated_impl(aux::index_sequence<Ns...>) const {
#if defined(__cpp_fold_expressions)
    return ((current_state_[Ns] ==
             aux::get_id<state_t, terminate_state>((states_ids_t*)0)) &&
            ...);
#else
    auto result = true;
...
class sm {
  ...
  using logger_t = typename Tsm::logger_policy::type;
  using logger_dep_t =
      aux::conditional_t<aux::is_same<no_policy, logger_t>::value,
                         aux::type_list<>, aux::type_list<logger_t&> >;
  using transitions_t = decltype(aux::declval<sm_t>().operator()());
  static_assert(concepts::composable<sm_t>::value,
 ...
 public:
  ...
  using events =
      aux::apply_t<aux::unique_t, aux::apply_t<get_all_events, transitions_t> >;
  using transitions = aux::apply_t<aux::type_list, transitions_t>;

 private:
  using sm_all_t = aux::apply_t<
      get_non_empty_t,
      aux::join_t<
          aux::conditional_t<
              aux::is_same<
                  no_policy,
                  typename Tsm::dont_instantiate_statemachine_class_policy>::
                  value,
              aux::type_list<sm_t>, aux::none_type>,
          aux::apply_t<get_sm_t, state_machines> > >;

  using sub_sms_t = aux::apply_t<
      aux::pool,
      typename convert_to_sm<
          Tsm, aux::apply_t<aux::unique_t,
                            aux::apply_t<get_sub_sms, states> > >::type>;
  using deps = aux::apply_t<merge_deps, transitions_t>;
  using deps_t = aux::apply_t<
      aux::pool,
      aux::apply_t<aux::unique_t,
                   aux::join_t<deps, sm_all_t, logger_dep_t,
                               aux::apply_t<merge_deps, sub_sms_t> > > >;
  struct events_ids : aux::apply_t<aux::inherit, events> {};

 public:
  constexpr sm() : deps_{aux::init{}, aux::pool<>{}}, sub_sms_{aux::pool<>{}} {
    aux::get<sm_impl<Tsm> >(sub_sms_).start(deps_, sub_sms_);
  }
  template <class TDeps,
            __BOOST_SML_REQUIRES(
                !aux::is_same<aux::remove_reference_t<TDeps>, sm>::value)>
  constexpr explicit sm(TDeps&& deps)
      : deps_{aux::init{}, aux::pool<TDeps>{deps}},
        sub_sms_{aux::pool<TDeps>{deps}} {
    aux::get<sm_impl<Tsm> >(sub_sms_).start(deps_, sub_sms_);
  }
  template <class... TDeps,
            __BOOST_SML_REQUIRES((sizeof...(TDeps) > 1) &&
                                 aux::is_unique_t<TDeps...>::value)>
  constexpr explicit sm(TDeps&&... deps)
      : deps_{aux::init{}, aux::pool<TDeps...>{deps...}},
        sub_sms_{aux::pool<TDeps...>{deps...}} {
    aux::get<sm_impl<Tsm> >(sub_sms_).start(deps_, sub_sms_);
  }
  constexpr sm(aux::init, deps_t& deps) : deps_{deps}, sub_sms_{deps} {
    aux::get<sm_impl<Tsm> >(sub_sms_).start(deps_, sub_sms_);
  }
  ...
  template <class TEvent,
            __BOOST_SML_REQUIRES(aux::is_base_of<TEvent, events_ids>::value)>
  constexpr bool process_event(const TEvent& event) {
    return aux::get<sm_impl<Tsm> >(sub_sms_).process_event(event, deps_,
                                                           sub_sms_);
  }
  template <class TEvent,
            __BOOST_SML_REQUIRES(!aux::is_base_of<TEvent, events_ids>::value)>
  constexpr bool process_event(const TEvent& event) {
    return aux::get<sm_impl<Tsm> >(sub_sms_).process_event(
        unexpected_event<_, TEvent>{event}, deps_, sub_sms_);
  }
  template <class T = aux::identity<sm_t>, class TVisitor,
            __BOOST_SML_REQUIRES(concepts::callable<void, TVisitor>::value)>
  constexpr void visit_current_states(const TVisitor& visitor) const {
    ...
    aux::cget<sm_impl_t>(sub_sms_).visit_current_states(
        visitor, states_t{}, aux::make_index_sequence<regions>{});
  }
  template <class T = aux::identity<sm_t>, class TState,
            __BOOST_SML_REQUIRES(sm_impl<typename Tsm::template rebind<
                                     typename T::type> >::regions == 1)>
  ...
  constexpr bool is(const TState&) const {
    ...
    visit_current_states<T>([&](auto state) {
      (void)state;
      result |=
          (aux::get_id<state_t, typename TState::type>((states_ids_t*)0) ==
           aux::get_id<state_t, typename decltype(state)::type>(
               (states_ids_t*)0));
    });
    return result;
  }
  ...
  template <
      class T = aux::identity<sm_t>, class... TStates,
      __BOOST_SML_REQUIRES(
          sizeof...(TStates) ==
          sm_impl<typename Tsm::template rebind<typename T::type> >::regions)>
  constexpr bool is(const TStates&...) const {
    ...
    auto i = 0;
    state_t state_ids[] = {
        aux::get_id<state_t, typename TStates::type>((states_ids_t*)0)...};
    visit_current_states<T>([&](auto state) {
      (void)state;
      result &= (aux::get_id<state_t, typename decltype(state)::type>(
                     (states_ids_t*)0) == state_ids[i++]);
    });
    return result;
  }
  template <class T = aux::identity<sm_t>, class... TStates,
            __BOOST_SML_REQUIRES(
                !aux::is_same<no_policy, typename Tsm::testing_policy>::value &&
                aux::always<T>::value)>
  constexpr void set_current_states(const TStates&...) {
    ...
    auto region = 0;
#if defined(__cpp_fold_expressions)
    ((sm.current_state_[region++] =
          aux::get_id<state_t, typename TStates::type>((states_ids_t*)0)),
     ...);
#else
    (void)aux::swallow{
        0, (sm.current_state_[region++] =
                aux::get_id<state_t, typename TStates::type>((states_ids_t*)0),
            0)...};
#endif
  }
...
template <class T, class E>
auto args__(int)
    -> aux::function_traits_t<decltype(&T::__BOOST_SML_TEMPLATE_KEYWORD
                                       operator()<back::get_event_t<E> >)>;
template <class T, class E>
auto args__(...) -> decltype(args1__<T, E>(0));
template <class T, class E>
using args_t = decltype(args__<T, E>(0));
template <class T, class TEvent, class Tsm, class TDeps>
constexpr decltype(auto) get_arg(const aux::type<T>&, const TEvent&, Tsm&,
                                 TDeps& deps) {
  return aux::get<T>(deps);
}
template <class TEvent, class Tsm, class TDeps>
constexpr decltype(auto) get_arg(const aux::type<TEvent>&, const TEvent& event,
                                 Tsm&, TDeps&) {
  return event;
}
template <class TEvent, class Tsm, class TDeps>
constexpr decltype(auto) get_arg(const aux::type<const TEvent&>&,
                                 const TEvent& event, Tsm&, TDeps&) {
  return event;
}
template <class T, class TEvent, class Tsm, class TDeps>
constexpr decltype(auto) get_arg(const aux::type<const TEvent&>&,
                                 const back::unexpected_event<T, TEvent>& event,
                                 Tsm&, TDeps&) {
  return event.event_;
}
template <class T, class TEvent, class Tsm, class TDeps>
constexpr decltype(auto) get_arg(const aux::type<const TEvent&>&,
                                 const back::on_entry<T, TEvent>& event, Tsm&,
                                 TDeps&) {
  return event.event_;
}
template <class T, class TEvent, class Tsm, class TDeps>
constexpr decltype(auto) get_arg(const aux::type<const TEvent&>&,
                                 const back::on_exit<T, TEvent>& event, Tsm&,
                                 TDeps&) {
  return event.event_;
}
template <class T, class TEvent, class Tsm, class TDeps>
constexpr decltype(auto) get_arg(const aux::type<const TEvent&>&,
                                 const back::exception<T, TEvent>& event, Tsm&,
                                 TDeps&) {
  return event.exception_;
}
template <class... TEvents, class TEvent, class Tsm, class TDeps>
constexpr decltype(auto) get_arg(const aux::type<back::defer<TEvents...> >&,
                                 const TEvent, Tsm& sm, TDeps&) {
  return back::defer<TEvents...>{sm.defer_};
}
template <class... TEvents, class TEvent, class Tsm, class TDeps>
constexpr decltype(auto) get_arg(const aux::type<back::process<TEvents...> >&,
                                 const TEvent, Tsm& sm, TDeps&) {
...
struct call<TEvent, aux::type_list<>, TLogger> {
  template <class T, class Tsm, class TDeps, class TSubs>
  constexpr static auto execute(T object, const TEvent& event, Tsm&,
                                TDeps& deps, TSubs&) {
    using result_type = decltype(object());
    return execute_impl<typename Tsm::sm_t>(aux::type<result_type>{}, object,
                                            event, deps);
  }
  template <class Tsm, class T, class TDeps>
  constexpr static auto execute_impl(const aux::type<bool>&, T object,
                                     const TEvent& event, TDeps& deps) {
    const auto result = object();
    back::policies::log_guard<Tsm>(aux::type<TLogger>{}, deps, object, event,
                                   result);
    return result;
  }
  template <class Tsm, class T, class TDeps>
  constexpr static auto execute_impl(const aux::type<void>&, T object,
                                     const TEvent& event, TDeps& deps) {
...
struct call<TEvent, aux::type_list<TEvent>, back::no_policy> {
  template <class T, class Tsm, class TDeps, class TSubs>
  constexpr static auto execute(T object, const TEvent& event, Tsm&, TDeps&,
                                TSubs&) {
    return object(event);
  }
};
...
struct call<TEvent, aux::type_list<action_base>, back::no_policy> {
  template <class T, class Tsm, class TDeps, class TSubs>
  static auto execute(T object, const TEvent& event, Tsm& sm, TDeps& deps,
                      TSubs& subs) {
...
struct call<TEvent, aux::type_list<action_base>, TLogger> {
  template <class T, class Tsm, class TDeps, class TSubs>
  constexpr static auto execute(T object, const TEvent& event, Tsm& sm,
                                TDeps& deps, TSubs& subs) {
    return object(event, sm, deps, subs);
  }
};
...
struct call<TEvent, aux::type_list<Ts...>, TLogger> {
  ...
  constexpr static auto execute(T object, const TEvent& event, Tsm& sm,
                                TDeps& deps, TSubs&) {
    using result_type =
        decltype(object(get_arg(aux::type<Ts>{}, event, sm, deps)...));
    return execute_impl<typename Tsm::sm_t>(aux::type<result_type>{}, object,
                                            event, sm, deps);
  }
...
class seq_ : operator_base {
 public:
  ...
  template <class TEvent, class Tsm, class TDeps, class TSubs>
  constexpr void operator()(const TEvent& event, Tsm& sm, TDeps& deps,
                            TSubs& subs) {
 ...
 private:
  ...
  constexpr void for_all(const aux::index_sequence<Ns...>&, const TEvent& event,
                         Tsm& sm, TDeps& deps, TSubs& subs) {
#if defined(__cpp_fold_expressions)
    (call<TEvent, args_t<Ts, TEvent>, typename Tsm::logger_t>::execute(
         aux::get_by_id<Ns>(&a), event, sm, deps, subs),
     ...);
#else
    (void)aux::swallow{
        0, (call<TEvent, args_t<Ts, TEvent>, typename Tsm::logger_t>::execute(
                aux::get_by_id<Ns>(&a), event, sm, deps, subs),
            0)...};
#endif
  }
...
class and_ : operator_base {
 public:
  ...
  template <class TEvent, class Tsm, class TDeps, class TSubs>
  constexpr auto operator()(const TEvent& event, Tsm& sm, TDeps& deps,
                            TSubs& subs) {
 ...
 private:
  template <int... Ns, class TEvent, class Tsm, class TDeps, class TSubs>
  constexpr auto for_all(const aux::index_sequence<Ns...>&, const TEvent& event,
                         Tsm& sm, TDeps& deps, TSubs& subs) {
#if defined(__cpp_fold_expressions)
    return (call<TEvent, args_t<Ts, TEvent>, typename Tsm::logger_t>::execute(
...
class not_ : operator_base {
 public:
  ...
  constexpr auto operator()(const TEvent& event, Tsm& sm, TDeps& deps,
                            TSubs& subs) {
    return !call<TEvent, args_t<T, TEvent>, typename Tsm::logger_t>::execute(
        g, event, sm, deps, subs);
  }
...
template <class T1, class T2,
          __BOOST_SML_REQUIRES(concepts::callable<bool, T1>::value&&
                                   concepts::callable<bool, T2>::value)>
constexpr auto operator&&(const T1& t1, const T2& t2) {
  return front::and_<aux::zero_wrapper<T1>, aux::zero_wrapper<T2> >(
      aux::zero_wrapper<T1>{t1}, aux::zero_wrapper<T2>{t2});
}
template <class T1, class T2,
          __BOOST_SML_REQUIRES(concepts::callable<bool, T1>::value&&
                                   concepts::callable<bool, T2>::value)>
constexpr auto operator||(const T1& t1, const T2& t2) {
  return front::or_<aux::zero_wrapper<T1>, aux::zero_wrapper<T2> >(
      aux::zero_wrapper<T1>{t1}, aux::zero_wrapper<T2>{t2});
}
template <class T1, class T2,
          __BOOST_SML_REQUIRES(concepts::callable<void, T1>::value&&
                                   concepts::callable<void, T2>::value)>
constexpr auto operator,(const T1& t1, const T2& t2) {
  return front::seq_<aux::zero_wrapper<T1>, aux::zero_wrapper<T2> >(
      aux::zero_wrapper<T1>{t1}, aux::zero_wrapper<T2>{t2});
}
namespace front {
namespace actions {
...
using dont_instantiate_statemachine_class =
    back::policies::dont_instantiate_statemachine_class;
#if defined(_MSC_VER) && !defined(__clang__)
template <class T, class... TPolicies,
...
auto transitional_impl(T&& t)
    -> aux::always<typename T::dst_state, typename T::src_state,
                   typename T::event, typename T::deps, decltype(T::initial),
                   decltype(T::history)>;
template <class T>
...
struct event {
  ...
  constexpr auto operator[](const T& t) const {
    return transition_eg<event, aux::zero_wrapper<T> >{*this,
                                                       aux::zero_wrapper<T>{t}};
  }
  template <class T, __BOOST_SML_REQUIRES(concepts::callable<void, T>::value)>
  constexpr auto operator/(const T& t) const {
    return transition_ea<event, aux::zero_wrapper<T> >{*this,
                                                       aux::zero_wrapper<T>{t}};
  }
...
struct state_impl {
  ...
  template <class T, __BOOST_SML_REQUIRES(concepts::callable<bool, T>::value)>
  constexpr auto operator[](const T& t) const {
    return transition_sg<TState, aux::zero_wrapper<T> >{
        static_cast<const TState&>(*this), aux::zero_wrapper<T>{t}};
  }
  template <class T, __BOOST_SML_REQUIRES(concepts::callable<void, T>::value)>
  constexpr auto operator/(const T& t) const {
    return transition_sa<TState, aux::zero_wrapper<T> >{
        static_cast<const TState&>(*this), aux::zero_wrapper<T>{t}};
  }
};
template <class TState>
struct state : state_impl<state<TState> > {
  ...
  constexpr auto operator()(const initial_state&) const {
    return state<TState(initial_state)>{};
  }
  constexpr auto operator()(const history_state&) const {
...
template <class TState>
struct state<TState(initial_state)>
    : state_impl<state<TState(initial_state)> > {
  using type = TState;
...
template <class TState>
struct state<TState(history_state)>
    : state_impl<state<TState(history_state)> > {
  using type = TState;
  static constexpr auto initial = true;
...
struct ignore<E, aux::type_list<Ts...> > {
  ...
  struct non_events {
    using type = aux::conditional_t<
        aux::is_same<
            back::get_event_t<E>,
            aux::remove_const_t<aux::remove_reference_t<T> > >::value ||
...
struct get_deps<
    T<Ts...>, E,
    aux::enable_if_t<aux::is_base_of<operator_base, T<Ts...> >::value> > {
...
struct transition<front::event<E>, G> {
  template <class T>
  constexpr auto operator/(const T& t) const {
    return transition<front::event<E>, G, aux::zero_wrapper<T> >{
        e, g, aux::zero_wrapper<T>{t}};
  }
  front::event<E> e;
  G g;
};
...
struct transition<state<S2>, G, A>
    : transition<state<internal>, state<S2>, front::event<back::anonymous>, G,
                 A> {
  using transition<state<internal>, state<S2>, front::event<back::anonymous>, G,
                   A>::g;
  using transition<state<internal>, state<S2>, front::event<back::anonymous>, G,
                   A>::a;
  constexpr transition(const G& g, const A& a)
      : transition<state<internal>, state<S2>, front::event<back::anonymous>, G,
                   A>{g, a} {}
  template <class T>
...
struct transition<state<S1>, state<S2> >
    : transition<state<S1>, state<S2>, front::event<back::anonymous>, always,
                 none> {
...
struct transition_sg<state<S2>, G>
    : transition<state<internal>, state<S2>, front::event<back::anonymous>, G,
                 none> {
  using transition<state<internal>, state<S2>, front::event<back::anonymous>, G,
                   none>::g;
  ...
  template <class T>
  constexpr auto operator/(const T& t) const {
    return transition<state<S2>, G, aux::zero_wrapper<T> >{
        g, aux::zero_wrapper<T>{t}};
  }
  template <class T>
  constexpr auto operator=(const T&) const {
    return transition<T, state<S2>, front::event<back::anonymous>, G, none>{
        g, none{}};
  }
};
template <class S2, class A>
struct transition_sa<state<S2>, A>
    : transition<state<internal>, state<S2>, front::event<back::anonymous>,
                 always, A> {
  using transition<state<internal>, state<S2>, front::event<back::anonymous>,
                   always, A>::a;
  ...
  template <class T>
  constexpr auto operator=(const T&) const {
    return transition<T, state<S2>, front::event<back::anonymous>, always, A>{
        always{}, a};
  }
};
...
struct transition<state<S2>, front::event<E> > {
  template <class T>
  constexpr auto operator=(const T&) const {
    return transition<T, state<S2>, front::event<E>, always, none>{always{},
                                                                   none{}};
  }
  const state<S2>& s2;
  front::event<E> e;
...
struct transition<state<S1>, transition<front::event<E>, G, A> >
  ...
  constexpr transition(const state<S1>&,
                       const transition<front::event<E>, G, A>& t)
      : transition<state<internal>, state<S1>, front::event<E>, G, A>{t.g,
                                                                      t.a} {}
  template <class T>
  constexpr auto operator=(const T&) const {
...
struct transition<state<S1>, transition_sg<state<S2>, G> >
  ...
  constexpr transition(const state<S1>&, const transition_sg<state<S2>, G>& t)
      : transition<state<S1>, state<S2>, front::event<back::anonymous>, G,
                   none>{t.g, none{}} {}
};
...
struct transition<state<S2>, transition_eg<front::event<E>, G> >
  ...
  constexpr transition(const state<S2>&,
...
struct transition<state<S1>,
                  transition<state<S2>, transition_eg<front::event<E>, G> > >
  ...
  constexpr transition(
      ...
      : transition<state<S1>, state<S2>, front::event<E>, G, none>{t.g,
                                                                   none{}} {}
};
...
constexpr void update_current_state(SM&, TDeps& deps, TSubs&,
                                    typename SM::state_t& current_state,
                                    const typename SM::state_t& new_state,
                                    const TSrcState&, const TDstState&) {
  back::policies::log_state_change<typename SM::sm_t>(
      aux::type<typename SM::logger_t>{}, deps,
...
constexpr void update_current_state(SM&, TDeps& deps, TSubs& subs,
                                    typename SM::state_t& current_state,
                                    const typename SM::state_t& new_state,
                                    const TSrcState&,
  ...
  current_state = new_state;
  update_composite_states<back::sm_impl<T> >(
      subs, typename back::sm_impl<T>::has_history_states{},
...
struct transition<state<S1>, state<S2>, front::event<E>, G, A> {
  ...
  using guard = G;
  using action = A;
  using deps = aux::apply_t<aux::unique_t,
                            aux::join_t<get_deps_t<G, E>, get_deps_t<A, E> > >;
  constexpr transition(const G& g, const A& a) : g(g), a(a) {}
  template <class TEvent, class SM, class TDeps, class TSubs>
  constexpr bool execute(const TEvent& event, SM& sm, TDeps& deps, TSubs& subs,
                         typename SM::state_t& current_state, aux::true_type) {
    if (call<TEvent, args_t<G, TEvent>, typename SM::logger_t>::execute(
            g, event, sm, deps, subs)) {
      sm.process_internal_event(back::on_exit<back::_, TEvent>{event}, deps,
                                subs, current_state);
      update_current_state(sm, deps, subs, current_state,
                           aux::get_id<typename SM::state_t, dst_state>(
                               (typename SM::states_ids_t*)0),
                           state<src_state>{}, state<dst_state>{});
      call<TEvent, args_t<A, TEvent>, typename SM::logger_t>::execute(
          a, event, sm, deps, subs);
      sm.process_internal_event(back::on_entry<back::_, TEvent>{event}, deps,
                                subs, current_state);
      return true;
    }
    return false;
  ...
  template <class TEvent, class SM, class TDeps, class TSubs>
  constexpr bool execute(const TEvent& event, SM& sm, TDeps& deps, TSubs& subs,
                         typename SM::state_t& current_state, aux::false_type) {
    if (call<TEvent, args_t<G, TEvent>, typename SM::logger_t>::execute(
            g, event, sm, deps, subs)) {
      update_current_state(sm, deps, subs, current_state,
                           aux::get_id<typename SM::state_t, dst_state>(
...
struct transition<state<S1>, state<S2>, front::event<E>, always, A> {
  ...
  constexpr bool execute(const TEvent& event, SM& sm, TDeps& deps, TSubs& subs,
                         typename SM::state_t& current_state, aux::true_type) {
    sm.process_internal_event(back::on_exit<back::_, TEvent>{event}, deps, subs,
                              current_state);
    update_current_state(sm, deps, subs, current_state,
                         aux::get_id<typename SM::state_t, dst_state>(
                             (typename SM::states_ids_t*)0),
...
struct transition<state<internal>, state<S2>, front::event<E>, G, none> {
  ...
  constexpr bool execute(const TEvent& event, SM& sm, TDeps& deps, TSubs& subs,
                         typename SM::state_t&, Ts&&...) {
    return call<TEvent, args_t<G, TEvent>, typename SM::logger_t>::execute(
        g, event, sm, deps, subs);
  }
  G g;
};
...
struct transition<state<S1>, state<S2>, front::event<E>, always, none> {
  ...
  template <class TEvent, class SM, class TDeps, class TSubs>
  constexpr bool execute(const TEvent&, SM& sm, TDeps& deps, TSubs& subs,
                         typename SM::state_t& current_state, aux::false_type) {
    update_current_state(sm, deps, subs, current_state,
                         aux::get_id<typename SM::state_t, dst_state>(
                             (typename SM::states_ids_t*)0),
...
struct transition<state<internal>, state<S2>, front::event<E>, always, none> {
  ...
  constexpr transition(const always&, const none&) {}
  template <class TEvent, class SM, class TDeps, class TSubs, class... Ts>
  constexpr bool execute(const TEvent&, SM&, TDeps&, TSubs&,
                         typename SM::state_t&, Ts&&...) {
    return true;
  }
  __BOOST_SML_ZERO_SIZE_ARRAY(aux::byte);
...
#endif
template <class TEvent>
__BOOST_SML_UNUSED front::event<back::on_entry<_, TEvent> > on_entry
    __BOOST_SML_VT_INIT;
template <class TEvent>
__BOOST_SML_UNUSED front::event<back::on_exit<_, TEvent> > on_exit
    __BOOST_SML_VT_INIT;
template <class TEvent>
front::event<back::unexpected_event<TEvent> > unexpected_event
    __BOOST_SML_VT_INIT;
template <class T>
...
constexpr auto operator""_s() {
  return []<auto... Ns>(aux::index_sequence<Ns...>) {
    return front::state<aux::string<char, Str.data[Ns]...> >{};
  }(aux::make_index_sequence<Str.size>{});
}
template <aux::fixed_string Str>
constexpr auto operator""_e() {
  return []<auto... Ns>(aux::index_sequence<Ns...>) {
    return event<aux::string<char, Str.data[Ns]...> >;
  }(aux::make_index_sequence<Str.size>{});
}
#elif !(defined(_MSC_VER) && !defined(__clang__))
...
template <class... Ts,
          __BOOST_SML_REQUIRES(
              aux::is_same<aux::bool_list<aux::always<Ts>::value...>,
                           aux::bool_list<
                               concepts::transitional<Ts>::value...> >::value)>
...
#elif defined(_MSC_VER) && !defined(__clang__) && \
    defined(__BOOST_SML_DEFINED_HAS_BUILTIN)
#undef __has_builtin
