...
class expectations
    : public std::unordered_map<std::type_index,
...
class mocks_provider : public config {
  ...
  struct mock_provider {
    ...
    template <class... Ts>
    struct transform<aux::type_list<Ts...>>
        : map<aux::type<typename given<Ts>::type>...> {};

    template <class T>
    using is_resolvable =
        resolve<aux::type<T>, transform<typename TInjector::deps>>;

    template <class T, class TInitialization, class TMemory, class... TArgs>
    std::enable_if_t<is_resolvable<T>::value || !std::is_polymorphic<T>::value,
                     T*>
...
auto& expect(TInjector& injector, R (T::*)(TArgs...)) {
  auto& expectations =
      ((di::core::injector__<TInjector>&)injector).cfg().expectations();
