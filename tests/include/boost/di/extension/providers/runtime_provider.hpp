...
class runtime_provider : public config {
  ...
  class abstract_provider : TErrorPolicy {
   public:
    explicit abstract_provider(const bindings_t& bindings)
        : bindings_{bindings} {}
...
template <class TErrorPolicy, class TScopeTraits>
class injector
    : public core::injector<runtime_provider<TErrorPolicy, TScopeTraits>> {
  template <class...>
  struct any;
 ...
 public:
  injector()
      : core::injector<runtime_provider<TErrorPolicy, TScopeTraits>>{
            core::init{}} {}
  ...
  template <class T,
            std::enable_if_t<!std::is_base_of<core::injector_base, T>::value,
                             int> = 0>
  void install(const T& binding) {
    this->cfg().bindings()[std::type_index(typeid(typename T::expected))] =
        [this, binding] { return make<typename T::given>(binding); };
  }

  template <
      class T,
      std::enable_if_t<std::is_base_of<core::injector_base, T>::value, int> = 0>
 ...
 private:
  ...
  void install(aux::type_list<>, const TInjector& injector,
               aux::identity<runtime_provider<TCfg, TST>>) {
  ...
  auto make(const TBinding&)
      -> decltype(make_impl<T>(typename ctor_traits<T>::type{})) {
...
using runtime_injector =
    detail::injector<assert_error_policy, di::extension::shared_config>;
