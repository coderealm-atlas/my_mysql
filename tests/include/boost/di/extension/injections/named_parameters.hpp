...
constexpr auto operator""_s() {
  return aux::integral_constant<
      std::int64_t, const_hash(chars<Chars...>{}, sizeof...(Chars) + 1)>{};
}
#if (__GNUC__ >= 9)
#pragma GCC diagnostic pop
#endif

std::int64_t constexpr const_hash(char const* input, std::int64_t m = 0,
                                  std::int64_t i = 0) {
  return *input && i < m ? static_cast<std::int64_t>(*input) +
                               33 * const_hash(input + 1, m, i + 1)
                         : 5381;
}

template <char C, char... Chars>
std::int64_t constexpr const_hash(const chars<C, Chars...>&, std::int64_t m = 0,
                                  std::int64_t i = 0) {
  return C && i < m ? static_cast<std::int64_t>(C) +
                          33 * const_hash(chars<Chars...>{}, m, i + 1)
                    : 5381;
}

std::int64_t constexpr const_hash(const chars<>&, ...) { return 5381; }

constexpr pair get_name_impl(const char* input, int begin, int n = 0,
                             int quote = 0) {
  return !*input || *input == ','
             ? pair{0, 0}
             : (quote == 2
                    ...
                    : (quote == 1 && *input == '"'
                       ...
                       : quote == 0 && *input == '"'
                           ? get_name_impl(input + 1, begin, n, quote + 1)
                           : get_name_impl(input + 1,
                                           quote == 0 ? begin + 1 : begin,
                                           quote == 1 ? n + 1 : n, quote)));
}

constexpr pair get_name(const char* input, int N, int c = 0, int i = 0) {
  return c == N
             ? get_name_impl(input + 1, i + 1)
             : (*input ? (*input == ',' ? get_name(input + 1, N, c + 1, i + 1)
                                        : get_name(input + 1, N, c, i + 1))
                       : pair{0, 0});
}
...
struct parse {
  ...
  using type = aux::conditional_t<
      ...
      named<aux::integral_constant<std::int64_t,
                                   const_hash(&T::str[name.begin], name.end)>,
            TArg>>;
};
...
struct args__ {
  using type = typename args_impl<R, aux::make_index_sequence<sizeof...(TArgs)>,
                                  TArgs...>::type;
};
...
#define $inject(T, ...)                                                  \
  struct boost_di_inject__ {                                             \
    ...
    using type = ::boost::di::extension::args_t<                         \
        decltype(::boost::di::extension::ctor__<boost_di_inject__>(0))>; \
  };                                                                     \
  T(__VA_ARGS__)
