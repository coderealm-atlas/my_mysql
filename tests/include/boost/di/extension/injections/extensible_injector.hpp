...
class dependency_proxy
    : core::dependency_base,
      public core::dependency_impl<core::dependency_concept<TExpected, TName>,
                                   dependency_proxy<TScope, TExpected, TGiven,
                                                    TName, TPriority, TCtor>> {
  using dependency_t =
      core::dependency<TScope, TExpected, TGiven, TName, TPriority, TCtor>;
 ...
 public:
  ...
  dependency_proxy(dependency_t& orig_dependency) noexcept
      : orig_dependency_(orig_dependency) {}
  dependency_proxy(dependency_proxy& other) noexcept
      : orig_dependency_(other.orig_dependency_) {}
  dependency_proxy(dependency_proxy&& other) noexcept
      : orig_dependency_(other.orig_dependency_) {}
  ...
  static decltype(scope_t::template try_create<T, Name>(
      aux::declval<TProvider>()))
  try_create(const TProvider&);

  template <class T, class Name, class TProvider>
  auto create(const TProvider& provider) {
    return static_cast<core::dependency__<dependency_t>&>(orig_dependency_)
        .template create<T, Name>(provider);
  }
...
auto make_extensible_impl(const aux::type<TDependency>&, TInjector& injector) {
  return dependency_proxy<
      typename TDependency::scope, typename TDependency::expected,
      typename TDependency::given, typename TDependency::name,
      typename TDependency::priority, typename TDependency::ctor>{injector};
}
