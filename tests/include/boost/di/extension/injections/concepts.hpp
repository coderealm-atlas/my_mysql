...
template <class T>
auto ctor__(int)
    -> aux::function_traits_t<decltype(&T::template ctor<_, _, _>)>;
...
#define BOOST_DI_INJECT_TRAITS_IMPL_1(...)                           \
  static void ctor(BOOST_DI_REPEAT(BOOST_DI_SIZE(__VA_ARGS__),       \
                                   BOOST_DI_GEN_CTOR, __VA_ARGS__)); \
  static void name(BOOST_DI_REPEAT(BOOST_DI_SIZE(__VA_ARGS__),       \
                                   BOOST_DI_GEN_NAME, __VA_ARGS__)); \
  using type BOOST_DI_UNUSED =                                       \
      detail::combine_t<decltype(ctor__<boost_di_inject__>(0)),      \
                        aux::function_traits_t<decltype(name)>>;
...
template <template <class...> class TWrapper, class T, class... Ts,
          template <class...> class X, class TScope, class Y>
auto concept_cast(const TWrapper<T, X<TScope, Y>>& arg) {
  return arg.wrapper_
      .operator concept_traits_t<T, aux::remove_qualifiers_t<Y>>();
}
