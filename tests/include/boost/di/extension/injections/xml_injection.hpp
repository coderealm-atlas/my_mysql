...
class inject_from_xml {
 public:
  ...
  auto operator()(const TInjector& injector,
                  const T&) const -> std::shared_ptr<typename T::expected> {
    auto parser = injector.template create<std::unique_ptr<ixml_parser>>();
    auto parsed = parser->parse(typeid(typename T::expected).name());
    return create_impl<typename T::expected>(injector, parsed,
                                             xml_list<TImpl...>{});
  }

 private:
  template <class I, class TInjector, class T, class... Ts>
  std::shared_ptr<I> create_impl(const TInjector& injector,
  ...
  std::shared_ptr<I> create_impl(TInjector&, const std::string&,
