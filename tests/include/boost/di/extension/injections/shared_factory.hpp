...
struct shared_factory_impl {
  shared_factory_impl(TFunc&& creation_func)
      : creation_func_(std::move(creation_func)), is_created_(false) {}

  shared_factory_impl(const shared_factory_impl& other)
      : creation_func_(std::move(other.creation_func_)),
  ...
  explicit shared_factory_impl(shared_factory_impl&& other) noexcept
      : shared_factory_impl(std::move(other),
                            std::lock_guard<std::mutex>(other.mutex_)) {}
  //<<synchronized move constructor>>
  shared_factory_impl(shared_factory_impl&& other,
                      const std::lock_guard<std::mutex>&) noexcept
      : creation_func_(std::move(other.creation_func_)),
  ...
  auto operator()(const TInjector& const_injector, const TDependency&) {
    if (!is_created_) {
...
#endif
      {
        ...
        using override_dep =
            core::dependency<scopes::unique, typename TDependency::expected, T,
                             no_name, core::override>;
        //<<rebind to avoid recursion>>
        auto& orig_injector = const_cast<TInjector&>(const_injector);
        const auto& rebound_injector =
            injector_rebinder<NoRecursion>{}.template rebind<override_dep>(
                orig_injector);
