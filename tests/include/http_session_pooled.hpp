...
template <class RequestBody, class ResponseBody, class Allocator = std::allocator<char>>
class http_session_pooled : public std::enable_shared_from_this<http_session_pooled<RequestBody, ResponseBody, Allocator>> {
  using Self = http_session_pooled<RequestBody, ResponseBody, Allocator>;

 public:
  using request_t = boost::beast::http::request<RequestBody, boost::beast::http::basic_fields<Allocator>>;
  using response_t = boost::beast::http::response<ResponseBody, boost::beast::http::basic_fields<Allocator>>;
  ...
  void run(callback_t cb) {
    ...
    pool_.acquire(acquire_origin, [self](boost::system::error_code ec, beast_pool::Connection::Ptr c) {
      if (ec || !c) return self->finish(std::nullopt, 1);
      self->conn_ = std::move(c);
      // If HTTP proxy specified and scheme is https, perform CONNECT then upgrade to TLS
 ...
 private:
  ...
  // Create CONNECT request directly in shared_ptr to avoid string_view lifetime issues
  connect_req_ = std::make_shared<http::request<http::empty_body>>();
  connect_req_->method(http::verb::connect);
  connect_req_->target(connect_authority_);  // This copies the string into the request
  ...
  std::visit(
        [this](auto& s) mutable {
          ...
          http::async_write(s, *sp->connect_req_, boost::asio::bind_executor(
                                         this->conn_->executor(),
  ...
  void do_proxy_read_response() {
    ...
    std::visit(
        [this](auto& s) {
          auto sp = this->shared_from_this();
          http::async_read_header(s, buffer_, *proxy_parser_, boost::asio::bind_executor(
                                                    this->conn_->executor(),
                                                    [sp](boost::system::error_code ec, std::size_t) {
  ...
  ssl_stream->async_handshake(
        ...
        boost::asio::bind_executor(conn_->executor(), [sp = this->shared_from_this()](boost::system::error_code ec) {
    ...
    std::visit(
        [sp](auto& s) {
          http::async_write(s, *sp->req_ptr_, boost::asio::bind_executor(
                                               sp->conn_->executor(),
                                               [sp](boost::system::error_code ec, std::size_t) {
  ...
  if (!parser_) parser_.emplace();
    // Optional body limits/streaming can be configured by caller by customizing ResponseBody
    auto sp = this->shared_from_this();
    std::visit(
        [sp](auto& s) {
          http::async_read(s, sp->buffer_, *sp->parser_, boost::asio::bind_executor(
                                                      sp->conn_->executor(),
                                                      [sp](boost::system::error_code ec, std::size_t) {
  ...
  std::chrono::seconds pool_io_timeout() const { return pool_.config().io_timeout; }
  std::chrono::seconds pool_handshake_timeout() const { return pool_.config().handshake_timeout; }
 ...
 private:
  ...
  std::optional<boost::beast::http::response_parser<ResponseBody, Allocator>> parser_;
  std::optional<boost::beast::http::response_parser<boost::beast::http::empty_body>> proxy_parser_;

  request_t req_{};
  std::shared_ptr<request_t> req_ptr_{};
  std::shared_ptr<boost::beast::http::request<boost::beast::http::empty_body>> connect_req_{};
